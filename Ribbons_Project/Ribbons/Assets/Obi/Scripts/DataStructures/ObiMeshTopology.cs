...
public class ObiMeshTopology : ScriptableObject
{
	...
	public void Generate(){
		...
		GCHandle meshInfoHandle = Oni.PinMemory(meshInfo);
		...
		Oni.GetHalfEdgeMeshInfo(halfEdgeMesh,meshInfoHandle.AddrOfPinnedObject());
		meshInfo = (Oni.MeshInformation)meshInfoHandle.Target;
		Oni.UnpinMemory(meshVertices);
		Oni.UnpinMemory(meshTriangles);
		Oni.UnpinMemory(meshInfoHandle);
	 ...
	 */
	public bool SplitVertex(Oni.Vertex vertex, Plane splitPlane, MeshBuffer meshBuffer, Vector3[] positions, List<int> particleIndices, out Oni.Vertex newVertex, out HashSet<int> updatedEdges, out HashSet<int> addedEdges){
