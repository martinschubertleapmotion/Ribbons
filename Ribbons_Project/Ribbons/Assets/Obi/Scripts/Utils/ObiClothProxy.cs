...
public class ObiClothProxy : MonoBehaviour {
	...
	public struct ParticleInfluence{
		...
		public Vector3 bindVector;		/**< Vector from the influece particle to this vertex in bind pose.*/
	}
	...
	public void BindToProxy(){
		...
		for (int i = 0; i < proxy.positions.Length; ++i){
			...
			bindPoses[i] = Quaternion.Inverse(proxyToLocal * proxy.sharedTopology.vertexOrientation[i]);
		}
