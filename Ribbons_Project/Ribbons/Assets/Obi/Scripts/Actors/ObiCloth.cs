...
public class ObiCloth : ObiActor
{
    ...
    private Quaternion[] orientation;							/**< Per particle reference orientation.*/
	...
	private void InitializeWithRegularMesh(){
		...
		for(int i = 0; i < positions.Length; i++){
			...
			ObiMeshTopology.HEVertex vertex = topology.heVertices[i];
			for(int j = 0; j < vertex.physicalIDs.Count; j++){
				meshVertices[vertex.physicalIDs[j]] = positions[i];
			}
		...
		UpdateAerodynamicTriangleNormals();
	...
	{		
		...
		restDensities = new float[topology.heVertices.Count];
		...
		for (int i = 0; i < topology.heVertices.Count; i++){
			...
			foreach (ObiMeshTopology.HEFace face in topology.GetNeighbourFacesEnumerator(vertex)){
				areaContribution[i] += face.area/face.edges.Length;
            }
			
			// Get the shortest neighbour edge, particle radius will be half of its length.
			float minEdgeLength = Single.MaxValue;
			foreach (ObiMeshTopology.HEHalfEdge edge in topology.GetNeighbourEdgesEnumerator(vertex)){
				minEdgeLength = Mathf.Min(minEdgeLength,Vector3.Distance(topology.heVertices[edge.startVertex].position,
					                                                     topology.heVertices[edge.endVertex].position));
			}

			active[i] = true;
			mass[i] = 0.05f;
			restDensities[i] = 1000;
		...
		for (int i = 0; i < edges.Count; i++){
			...
			ObiMeshTopology.HEHalfEdge hedge = topology.heHalfEdges[edges[i].halfEdgeIndex];
			ObiMeshTopology.HEVertex startVertex = topology.heVertices[hedge.startVertex];
			ObiMeshTopology.HEVertex endVertex = topology.heVertices[hedge.endVertex];
			
			distanceConstraints.AddConstraint(true,hedge.startVertex,hedge.endVertex,Vector3.Distance(startVertex.position,endVertex.position),1,1);
		...
		for (int i = 0; i < topology.heFaces.Count; i++){
			ObiMeshTopology.HEFace face = topology.heFaces[i];
			aerodynamicConstraints.AddConstraint(true,topology.heHalfEdges[face.edges[0]].endVertex,
			                                     	  topology.heHalfEdges[face.edges[1]].endVertex,
			                                     	  topology.heHalfEdges[face.edges[2]].endVertex,
			                                     	  Vector3.up,
				                                      aerodynamicConstraints.windVector,
				                                      aerodynamicConstraints.airDensity,
		...
		if (topology.IsClosed){
			...
			int[] volumeIndices = new int[topology.heFaces.Count * 3];
			for (int i = 0; i < topology.heFaces.Count; i++){
				ObiMeshTopology.HEFace face = topology.heFaces[i];
				volumeIndices[i*3] = topology.heHalfEdges[face.edges[0]].endVertex;
				volumeIndices[i*3+1] = topology.heHalfEdges[face.edges[1]].endVertex;
				volumeIndices[i*3+2] = topology.heHalfEdges[face.edges[2]].endVertex;
			}

			int[] particleIndices = new int[topology.heVertices.Count];
			for (int i = 0; i < topology.heVertices.Count; i++){
				particleIndices[i] = i;
			}

			volumeConstraints.AddConstraint(true,particleIndices,volumeIndices,topology.MeshVolume,1,1);
		}
		...
		for (int i = 0; i < topology.heVertices.Count; i++){
			...
			foreach (ObiMeshTopology.HEVertex n1 in topology.GetNeighbourVerticesEnumerator(vertex)){
	
				float cosBest = 0;
				ObiMeshTopology.HEVertex vBest = n1;
	
				foreach (ObiMeshTopology.HEVertex n2 in topology.GetNeighbourVerticesEnumerator(vertex)){
	...
	public void CommitResultsToMesh()
	{
		...
		if (skinnedMeshRenderer != null){ //Update skinned mesh:
			...
			for(int i = 0; i < particleIndices.Count; ++i){
				...
				if (active[i]){
					...
					for(int j = 0; j < vertex.physicalIDs.Count; ++j){
						...
						meshVertices[vertex.physicalIDs[j]] = skinnedMeshRenderer.rootBone.InverseTransformPoint(position);
					}
				}	
			...
			for(int i = 0; i < particleIndices.Count; ++i){
				...
				ObiMeshTopology.HEVertex vertex = topology.heVertices[i];
				for(int j = 0; j < vertex.physicalIDs.Count; ++j){
					meshVertices[vertex.physicalIDs[j]] = position;
				}
	...
	private void UpdateNormalsAndTangents(){
		...
		if (skinnedMeshRenderer != null){
			...
			// Update normals:
			meshNormals = clothMesh.normals = topology.AreaWeightedNormals(meshNormals,meshVertices);
	...
	private void UpdateOrientations(){
		...
		if (orientation == null)
			orientation = new Quaternion[sharedTopology.heVertices.Count];  

		foreach(ObiMeshTopology.HEVertex vertex in sharedTopology.heVertices){
			Quaternion current = sharedTopology.CalculateVertexOrientation(vertex.index,meshNormals,meshVertices);
			orientation[vertex.index] = current;
		}
	...
	private void UpdateTangentSpace(){
		...
		foreach(ObiMeshTopology.HEVertex vertex in sharedTopology.heVertices){
			...
			Quaternion reference = sharedTopology.vertexOrientation[vertex.index];
			Quaternion current = orientation[vertex.index];
			...
			for (int i = 0; i < vertex.physicalIDs.Count; ++i){
				index = vertex.physicalIDs[i];
				oldw = tangents[index].w;
				tangents[index] = delta * tangents[index];
				tangents[index].w = oldw;
			}
	...
	private void UpdateAerodynamicTriangleNormals(){
		...
		for (int i = 0; i < topology.heFaces.Count; i++){
			...
			ObiMeshTopology.HEVertex v1 = topology.heVertices[topology.heHalfEdges[face.edges[0]].endVertex];
			ObiMeshTopology.HEVertex v2 = topology.heVertices[topology.heHalfEdges[face.edges[1]].endVertex];
			ObiMeshTopology.HEVertex v3 = topology.heVertices[topology.heHalfEdges[face.edges[2]].endVertex];

			if (active[v1.index] || active[v2.index] || active[v3.index]){
			aerodynamicConstraints.aerodynamicNormals[i] = transform.TransformDirection((meshNormals[v1.physicalIDs[0]]+
														   								 meshNormals[v2.physicalIDs[0]]+
														   								 meshNormals[v3.physicalIDs[0]]).normalized);
			}
	...
	public override void ResetActor(){
		...
		PushDataToSolver(new ObiSolverData(ObiSolverData.ParticleData.POSITIONS | ObiSolverData.ParticleData.PREVIOUS_POSITIONS | 
						 				   ObiSolverData.ParticleData.PREDICTED_POSITIONS | ObiSolverData.ParticleData.VELOCITIES));
	...
	private void ApplyTearing(){
		...
		if (tearedEdges.Count > 0){
			...
			// Removing this actor's distance constraints from the solver and then re-adding them at the end might not
	...
	public void GrabSkinAnimation(){
		...
		if (skinnedMeshRenderer != null){
			...
			for(int i = 0; i < particleIndices.Count; i++){
				...
				int vindex = topology.heVertices[i].physicalIDs[0];

				if (solver.inverseMasses[particleIndices[i]] == 0)
					solver.positions[particleIndices[i]] = skinnedMeshRenderer.transform.TransformPoint(meshVertices[vindex]);
	 ...
	 */
	public void Tear(int constraintIndex,MeshBuffer buffer){
		...
		Vector3 v1 = topology.heVertices[splitIndex].position;
		Vector3 v2 = topology.heVertices[intactIndex].position;
		Plane splitPlane = new Plane((v2-v1).normalized,v1);

		ObiMeshTopology.HEVertex newVertex;
		HashSet<int> updatedEdges; 
		
		// Try to split the vertex at that particle:
		if (!topology.SplitVertex(topology.heVertices[splitIndex],splitPlane,buffer,out newVertex,out updatedEdges)){
		...
		Array.Resize(ref restDensities,restDensities.Length+1);
		...
		solver.positions[newParticle[0]] = solver.positions[particleIndices[splitIndex]];
		solver.predictedPositions[newParticle[0]] = solver.predictedPositions[particleIndices[splitIndex]];
		solver.previousPositions[newParticle[0]] = solver.previousPositions[particleIndices[splitIndex]];
		solver.velocities[newParticle[0]] = solver.velocities[particleIndices[splitIndex]];
		
		solver.inverseMasses[newParticle[0]] = invMasses[invMasses.Length-1] = invMasses[splitIndex];
		solver.restDensities[newParticle[0]] = restDensities[restDensities.Length-1] = restDensities[splitIndex];
		solver.solidRadii[newParticle[0]] = solidRadii[solidRadii.Length-1] = solidRadii[splitIndex];
		solver.phases[newParticle[0]] = phases[phases.Length-1] = phases[splitIndex];
		...
		// calculate amount of new distance constraints:
		int newConstraints = 0;
		foreach (int halfEdgeIndex in updatedEdges){
			// if the half edge has no constraint in the edges->constraints map, it needs a new constraint.
			if (constraintHalfEdgeMap[halfEdgeIndex] < 0)  
				newConstraints++;
		}
		...
		foreach (int halfEdgeIndex in updatedEdges){
			...
			int cIndex = constraintHalfEdgeMap[halfEdgeIndex];

			if (cIndex < 0){ //new constraint
                ...
                distanceConstraints.AddConstraint(true,topology.heHalfEdges[halfEdgeIndex].startVertex,
			...
			}else{ //existing constraint
				
				// re-wire the existing distance constraint.
				distanceConstraints.springIndices[cIndex*2] = topology.heHalfEdges[halfEdgeIndex].startVertex;
				distanceConstraints.springIndices[cIndex*2+1] = topology.heHalfEdges[halfEdgeIndex].endVertex;
	...
	public override bool GenerateTethers(int maxTethers){
		...
		if (maxTethers > 0){
			...
			for (int i = 0; i < topology.heVertices.Count; i++){
				...
				foreach (ObiMeshTopology.HEVertex n in topology.GetNeighbourVerticesEnumerator(vertex)){
			...
			for (int i = 0; i < invMasses.Length; ++i){
				...
				for (int k = 0; k < Mathf.Min(maxTethers,tethers.Count); ++k){
					int tetherIndex = tethers[k].Value;
					tetherConstraints.AddConstraint(true,i,tetherIndex,Vector3.Distance(topology.heVertices[i].position,
					                                                                    topology.heVertices[tetherIndex].position),1,1);
				}
			}
	...
	public bool ReadParticlePropertyFromTexture(Texture2D source,Action<int,Color> onReadProperty){
		...
		for (int i = 0; i < topology.heVertices.Count; ++i){
			...
			for (int j = 0; j < vertex.physicalIDs.Count; ++j){
				Vector2 uv = clothMesh.uv[vertex.physicalIDs[j]];
				try{
			...
			color /= vertex.physicalIDs.Count;
	...
	public override Quaternion GetParticleOrientation(int index){
		if (orientation == null) 
			return topology.vertexOrientation[index];
