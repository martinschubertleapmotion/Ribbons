...
public class ObiCloth : ObiActor
{
	...
	{		
		...
		if (topology.IsClosed){
			...
			for (int i = 0; i < topology.heFaces.Count; i++){
				...
				volumeIndices[i*3] = e1.endVertex;
				volumeIndices[i*3+1] = e2.endVertex;
				volumeIndices[i*3+2] = e3.endVertex;
			}
		...
		if (skinnedMeshRenderer != null){ //Update skinned mesh:
			...
			for(int i = 0; i < particleIndices.Count; ++i){
				...
				if (active[i]){
					...
					foreach (int j in topology.visualVertexBuffer[i]){
						isClothVertex[j] = true;
						meshVertices[j] = skinnedMeshRenderer.rootBone.InverseTransformPoint(position);
					}
				}	
	...
	private void UpdateTangentSpace(){
		...
		foreach(Oni.Vertex vertex in topology.heVertices){

			Quaternion reference = topology.referenceOrientation[vertex.index];
			...
			foreach (int j in topology.visualVertexBuffer[vertex.index]){
				oldw = tangents[j].w;
				tangents[j] = delta * tangents[j];
				tangents[j].w = oldw;
			}
	...
	public void Tear(int constraintIndex,MeshBuffer buffer){
		...
		Vector3 v1 = solver.positions[particleIndices[splitIndex]];
		Vector3 v2 = solver.positions[particleIndices[intactIndex]];
		...
		if (!topology.SplitVertex(topology.heVertices[splitIndex],splitPlane,buffer,solver.positions,particleIndices,out newVertex,out updatedEdges, out newEdges)){
