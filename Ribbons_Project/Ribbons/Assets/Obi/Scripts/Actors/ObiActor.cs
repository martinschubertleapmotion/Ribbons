...
public class ObiActor : MonoBehaviour, IObiSolverClient
{
	...
	[HideInInspector] public float[] restDensities;				/**< Particle rest densities*/
	...
	public virtual void OnEnable(){
		...
		for (int i = 0; i < particleIndices.Count; ++i){
			...
			if (!active[i])
				solver.inactiveParticles.Add(k);
            else
                solver.inactiveParticles.Remove(k);
		}
	 ...
	 */
	public void UpdateParticlePhases(){
	...
	public virtual void PushDataToSolver(ObiSolverData data){
		...
		for (int i = 0; i < particleIndices.Count; i++){
			...
			if ((data.particleData & ObiSolverData.ParticleData.REST_DENSITIES) != 0)
				solver.restDensities[k] = restDensities[i];
