Shader "Obi/Particles" {
	...
	SubShader { 
		...
		Pass { 
			...
			{ 
				v2f o;
				o.worldpos = mul(UNITY_MATRIX_MV, v.pos) + float4(v.corner.x, v.corner.y, 0, 0);
				o.pos = mul(UNITY_MATRIX_P, o.worldpos);
			...
			fout frag(v2f i) : SV_Target
			{
				...
				float4 pixelPos = float4(i.worldpos + n * sphereRadius, 1.0f); 
		...
		Pass {
        	Name "ShadowCaster"
				...
				v2f vert( vin v )
				{
					...
					o.worldpos = mul(UNITY_MATRIX_MV, v.pos) + float4(v.corner.x, v.corner.y, 0, 0);
					o.hpos = mul(UNITY_MATRIX_P, o.worldpos);
